...
'''
#golbal_frame = frame()
curve( pos=[(0, 0, 0), (0, 0, 250)], color=color.red)
curve( pos=[(0, 0, 0), (0, 250, 0)], color=color.green)
curve(pos=[(0, 0, 0), (250, 0, 0)], color=color.blue)
...
def create_legs(i):
    ...
    coxa_frame=cylinder( pos=vector(body_x/2,body_y/2,body_z/2),  axis=vector(coxa_len, 3, 10),radius=6, color=color.yellow)

    #femur_frame = frame(fpos=(coxa_len, 0, 0), axis=(1, 0, 0))
    femur_frame= cylinder( pos=vector(0,0,0), axis=vector(1, femur_len, 10),  radius=6, color=color.green)

    #tibia_frame = frame(fpos=(femur_len, 0, 0), axis=(0, 0, -1))
    tibia_frame= cylinder(pos=vector(0,0,0), axis=vector(1, 1, -tibia_len),  radius=6, color=color.blue)
...
def draw_legs(leg, a, b, g):
    ...
    coxa[leg].axis = vector(cos(g)*draw_x_offset*x_dir, sin(g)*draw_y_offset * y_dir, 0)
    femur[leg].axis = vector(cos(a)*draw_x_offset*x_dir, sin(g)*draw_y_offset * y_dir, -(z_dir*femur_len*sin(a)))
    tibia[leg].axis = vector(-cos(b)*x_dir, y_dir, tibia_len*-sin(b))
    coxa[leg].pos = vector(x_dir * draw_x_offset, y_dir * draw_y_offset, draw_z_offset)
    femur[leg].pos = coxa[leg].pos+coxa[leg].axis#vector(cos(a)*x_dir * (draw_y_offset+coxa_len), y_dir*(draw_y_offset+coxa_len), draw_z_offset)
    tibia[leg].pos = femur[leg].pos+femur[leg].axis#vector(x_dir*(draw_x_offset+femur_len*cos(a)+coxa_len), y_dir * draw_y_offset, draw_z_offset+(-(z_dir*femur_len*sin(a))))
...
, x_range - x_offset, y_start + y_step, z_ground)
'walk',x_range,x_offset, y_start, y_step, z_ground)   66.0 0 0 42.6 -27
'''
...
def walk_init():
    ...
    print( 'walk',x_range,x_offset, y_start, y_step, z_ground)
    wait_all_reach()
...
label(pos=vector(0, 210, z_stand_draw), text='Y')
label(pos=vector(210, 0, z_stand_draw), text='X')
label(pos=vector(0, 0, 210), text='Z')
...
#body_frame = frame()
print('body',body_z)
body = box(pos=vector(0, 0, (float(body_z/2))), length=body_x, height=body_y, width=body_z)
